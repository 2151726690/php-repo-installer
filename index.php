<?php
$settings = array(
	'repo'		=> 'skywalker512/FlarumChina',
	'videoId'	=> 'vDw1K-Spm_E',
	'logoUrl'	=> 'https://www.flarumchina.org/img/logo.png',
	'timeout'	=> array(
		'start' => 10,
		'redirection' => 10
	),
...
function fetch_url($url, $file=NULL) {
	...
	if($fn == 'curl') {
		...
		curl_setopt($ch, CURLOPT_USERAGENT, 'FlarumChina/php-repo-installer');
		...
		if($file) {
			...
			$out = @fopen($file, 'wb');
			if(!$out) {
				throw new Exception("不能够打开 " . __FUNCTION__ . "() 文件，我们没有读写权限。");
			}
		...
		$context = stream_context_create([
			'http' => [
				'ignore_errors' => TRUE,
				'method'		=> 'GET',
				'header'		=> 'User-agent: FlarumChina/php-repo-installer' . "\r\n"
			],
		]);
...
<body>
	...
	<div id="background" style="background-image: url(https://i.loli.net/2017/08/10/598bfdc9d668c.jpg);">
		
		<?php if(isset($settings['logoUrl'])) { ?><img class="logo" src="<?php echo $settings['logoUrl']; ?>"><?php } ?>
	</div>

<div id="terminal">FlarumChina PHP 安装器


接下来将会安装 <?php echo $settings['repo']; ?> 到 <?php echo ROOT_PATH; ?> 

<noscript><span class="status status--error">ERROR</span> 需要使用 JavaScript 来进行安装 ！</noscript></div>
		...
		$(function(){
			...
			var TOASTY = {
				...
				call: function() {
					var $this = this;
					this.remove();
					$("#background").append($('<img id="'+this.vars.selector.replace("#", "")+'-image" src="'+this.vars.image+'" />').css({
						position: "fixed",
			...
			var INSTALL = {
				...
				download: function() {
					var _this = this;
					writeConsoleCommand("已经下载 %s 从服务器上".replace("%s", settings.repo));
					$.ajax({
						url: url,
						...
						console.log(data)
						if(!XHR) {
							writeConsoleResponse("不能够连接到 %s".replace("%s", url), false);
							return;
						}
						if(data.status_code == 200) {
							_this.vars.target_filename = data.download.filename;
							writeConsoleResponse("■■■■■■■■■■ 100% - 下载完成保存为 %s".replace("%s", data.download.filename));
							_this.extract();
						} else {
				...
				extract: function(callback) {
					var _this = this;
					writeConsoleCommand("解压文件中，这需要一点时间！");
					$.ajax({
						url: url,
						...
						if(data.status_code == 200) {
							TOASTY.call();
							writeConsoleResponse("■■■■■■■■■■ 100% - 全部完成");
							writeConsoleResponse("恭喜 " + settings.repo + " 完成安装！");
							setTimeout(function() {
								writeLineBreak();
								writeLine("进程结束!");
								if(settings.redirect) {
									writeLineBreak();
									writeLine(['还有 ' + (typeof settings.timeout.redirection == "number" ? ('<span class="countdown">' + settings.timeout.redirection + '</span>')  + ' 秒跳转到安装界面' : '就是现在')], function() {
			...
			writeLine(['你还有 ' + (typeof settings.timeout.start == "number" ? ('<span class="countdown">' + settings.timeout.start + '</span>') + ' 秒的的时间考虑，若不想安装关闭即可！' : '就是现在')], function() {
				writeLineBreak(1);
